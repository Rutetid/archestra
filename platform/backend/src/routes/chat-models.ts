...
async function fetchZhipuaiModels(apiKey: string): Promise<ModelInfo[]> {
  const baseUrl = config.chat.zhipuai?.baseUrl || config.llm.zhipuai.baseUrl;
  const url = `${baseUrl}/models`;
  ...
  // Note: codegeex- models are part of a separate code generation API
  ...
  return data.data
    ...
    .map((model) => ({
      id: model.id,
      displayName: model.id, // Zhipuai doesn't provide display names
...
async function fetchModelsForProvider({
  ...
  try {
    ...
    if (["anthropic", "openai", "zhipuai"].includes(provider)) {
      if (apiKey) {
